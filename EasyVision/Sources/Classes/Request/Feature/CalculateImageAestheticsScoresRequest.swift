    ...
    public override func makeVNRequest(context: VisionRequestContext, completion: @escaping (Result<[ImageAestheticsResult], Error>) -> Void) -> VNRequest {
        if #available(iOS 18.0, *) {
            ...
            let req = VNRequest { _, _ in
                completion(.failure(EasyVisionError.visionError(NSError(domain: "EasyVision", code: -1, userInfo: [NSLocalizedDescriptionKey: "VNCalculateImageAestheticsScoresRequest requires iOS 18.0+"]))))
            }
            return req
        }
    }
